...
export default async function AdminPage() {
  ...
  const groups = Array.from(new Set(groupMatches.map((match) => match.group_name)));
  const knockoutResultGroups = roundOrder
    .map((roundId) => ({
      roundId,
      label: roundLabels[roundId],
      matches: matches.filter((match) => (match.stage ?? "group") === roundId),
    }))
    .filter((round) => round.matches.length > 0);
  const groupTeams = groups.map((group) => ({
    group,
    teams: Array.from(new Set(groupMatches.filter((match) => match.group_name === group).flatMap((match) => [match.home_team, match.away_team]))),
  }));
      ...
      <div className="admin-layout">
        ...
        <section className="panel admin-results-panel">
          ...
          <div className="result-rounds">
            {knockoutResultGroups.map((round) => (
              <section className="result-group result-group-major" key={round.roundId}>
                <h3>{round.label}</h3>
                <div className="result-list">
                  {round.matches.map((match) => (
                    <form className="result-row" action={updateResultAction} key={match.id}>
                      ...
                      <input name="homeScore" aria-label={`${match.home_team || match.home_source} score`} type="number" min="0" max="30" defaultValue={match.home_score ?? ""} />
                      <span className="score-separator">-</span>
                      <input name="awayScore" aria-label={`${match.away_team || match.away_source} score`} type="number" min="0" max="30" defaultValue={match.away_score ?? ""} />
                      ...
                      {match.home_team && match.away_team ? (
                        <select className="winner-select" name="winnerTeam" defaultValue={match.winner_team ?? ""} aria-label={`Qualifié match ${match.match_no}`}>
                          ...
                          <option value={match.home_team}>{teamName(match.home_team)}</option>
                          <option value={match.away_team}>{teamName(match.away_team)}</option>
                        </select>
                      ) : (
                        <span className="winner-placeholder">Qualifié à définir</span>
                      )}
          ...
          <details className="result-archive">
            <summary>
              <span>Phase de groupes</span>
              <small>{groupMatches.length} matchs déjà rangés par groupe</small>
            </summary>
                    ...
                    {groupMatches
                      ...
                      .map((match) => (
                        <form className="result-row" action={updateResultAction} key={match.id}>
                          ...
                          <span className="winner-placeholder" aria-hidden="true" />
                          <button className="icon-button" title="Sauvegarder le résultat" type="submit">
                            <Save size={17} />
                          </button>
                        </form>
        ...
        <div className="admin-utility-grid">
          <section className="panel admin-compact-panel">
            <div className="section-title">
              <div>
                <h2>Comptes</h2>
                <p className="muted">Noms, mots de passe et suppression.</p>
              </div>
              <ShieldCheck size={20} />
            </div>
            ...
            <section className="panel admin-compact-panel">
              <div className="section-title">
                <div>
                  ...
                  <p className="muted">Identifiant, nom affiché, mot de passe.</p>
                </div>
                <Plus size={20} />
              </div>
            ...
            <section className="panel admin-compact-panel">
              <div className="section-title">
                <div>
                  ...
                  <p className="muted">Joueurs, modèle et books.</p>
                </div>
              </div>
